...
async def put_clients(client: mct.Client, db: Session = Depends(database.get_db)):
    person: mdb.Client = db.query(mdb.Client).filter(
        mdb.Client.client_id == client.client_id).first()
    if not person:
...
async def delete_client(client_id: int, db: Session = Depends(database.get_db)):
    client: mdb.Client = db.query(mdb.Client).filter(
        mdb.Client.client_id == client_id).first()
    if not client:
        return JSONResponse(status_code=404, content={"message": "Пользователь не найден"})
    db.delete(client)
    db.commit()
